...
function TodoBlock() {
    ...
    if (recordId !== undefined && recordId !== null) {
        inventoryRecord = useRecordById(table, recordId);
    }

    const inventories = records ? records.map( (record, config)  => {
        return <Inventory key={record.id} record={record}  config={globalConfig}/>;
    }) : null;

    // Retrieve transaction table records
    const transactionTable = base.getTableByName('Inventory Transactions');
    const rewardRecords = useRecords(transactionTable);

    var filteredTransactionRecords = [];
    console.log(inventoryRecord);
    for (const record of rewardRecords) {
        if (inventoryRecord !== undefined && inventoryRecord !== null) {
            if(record.getCellValue('ID (from Inventory)') == inventoryRecord.getCellValue('ID')) {
                filteredTransactionRecords.push(record);
            }
        }
    }

    const transactions = filteredTransactionRecords ? Transactions(filteredTransactionRecords) : null;

    // Retrieve deals table records
    const dealTable = base.getTableByName('Inventory Deals');
    ...
    for (const record of dealRecords) {
        if (inventoryRecord !== undefined && inventoryRecord !== null) {
            console.log()
            if(record.getCellValueAsString('InventoryName (from Inventory)').includes(inventoryRecord.getCellValueAsString('InventoryName'))) {
    ...
    const activedeals = filteredDealRecords ? ActiveDeals(filteredDealRecords) : null;
       ...
       // Retrieve deals table records
       const recommendationsTable = base.getTableByName('Inventory Deals Recommendations');
    ...
    if ( detailsMode === true) {
       return  <div>
           <h1 style={{'margin':'12px'}}> Inventory </h1>
        {inventories}</div>;
    } else {
        return  <div style={{'margin':'12px'}}>
                    ...
                    <div class="card"
                            ...
                            <span style={{'float':'right', 'margin-right':'20px', 'display':' inline-flex', 'align-items': 'center', 'font-weight': '500', 'color': '#4669db', 'margin-top':'5px'}}>{inventoryRecord.getCellValue("Count") + " items"}{badge}</span>
                            ...
                            <h1> {inventoryRecord.getCellValue('InventoryName')} </h1>
                            <div style={{'margin-bottom': '32px'}}>
                                <span
                                    style={{ 'fontFamily' : '-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Helvetica,Arial,sans-serif',
                                        ...
                                        ...(inventoryRecord.getCellValue("StockAvailability").color === "blueBright" && {'background-color':  '#a6c8ff'}),
                                        ...(inventoryRecord.getCellValue("StockAvailability").color === "tealBright" && {'background-color':  '#9ef0f0'}),
                                        ...(inventoryRecord.getCellValue("StockAvailability").color === "purpleBright" && {'background-color':  '#d4bbff'}),
                                        'padding': '5px 10px',
                                        'border-radius': '10rem'                                        
                                    }}> 
                                    {inventoryRecord.getCellValue("StockAvailability").name}
                                </span>
                            </div>
...
function Inventory({record, config}) {
    ...
    return (
        <div class="card"
           ...
           >
             <span style={{'float':'right', 'margin-right':'20px', 'margin-top': '5px', 'display':' inline-flex', 'align-items': 'center', 'font-weight': '500', 'color': '#4669db'}}>{record.getCellValue("Count") + " items"}{badge}</span>
             <h1>{record.getCellValue('InventoryName') || 'Unnamed record'}</h1>
             <span
                style={{ 'fontFamily' : '-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Helvetica,Arial,sans-serif',
                    'font-weight': '700',
                    'letter-spacing': '1.5px',
                    ...
                    'font-size': '10px',
                    'background-color':  '#a6c8ff', 
                    ...(record.getCellValue("StockAvailability").color === "blueBright" && {'background-color':  '#a6c8ff'}),
                    ...(record.getCellValue("StockAvailability").color === "tealBright" && {'background-color':  '#9ef0f0'}),
                    ...(record.getCellValue("StockAvailability").color === "purpleBright" && {'background-color':  '#d4bbff'}),
                    'padding': '5px 10px',
                    'border-radius': '10rem'
                }}> 
                    ...
                    {linkedRecords.map(linkedRecord => {
                        return (
                            <tr key={linkedRecord.id} style={{borderTop: '2px solid #ddd'}}>
                                ...
                                <td style={{width: '25%', padding: '8px'}}>
                                <Text marginRight={3}>
                                    {linkedRecord.getCellValue('Count')}
                                    </Text>
                                </td>
                                <td style={{width: '25%', padding: '8px'}}>
                                <Text marginRight={3}>
                                    {linkedRecord.getCellValue('Transaction Type').name}
                                    </Text>
                                </td>
                            </tr>
                        );
